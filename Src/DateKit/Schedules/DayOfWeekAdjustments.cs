...
/// </summary>
public sealed class DayOfWeekAdjustments
{
	internal static readonly SByte[] DefaultArray = new SByte[Date.DaysPerWeek];
	...
	public Int32 this[DayOfWeek dayOfWeek]
	{
		...
		set
		{
			...
			if (unchecked((UInt32)index) >= array.Length)
				ThrowHelper.ThrowArgumentOutOfRangeException(dayOfWeek, ExceptionArgument.dayOfWeek);
			if (value is <= -Date.DaysPerWeek or >= Date.DaysPerWeek)

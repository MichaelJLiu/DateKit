...
public class AnnualDayOfMonthRecurrenceTests
{
		...
		Int32 month, Int32 day, Date date, Date[] expectedResults)
	{
		...
		AnnualDayOfMonthRecurrenceOptions options =
			new()
			{
				StartYear = 2000,
				EndYear = 2002,
				DayOfWeekAdjustments = new DayOfWeekAdjustments { [DayOfWeek.Saturday] = 2 },
			};
		Schedule schedule = new AnnualDayOfMonthRecurrence(month, day, options);
	...
	private static IEnumerable<Object[]> GetEnumerateForwardFromTestCases()
	{
		static Object[] CreateTestCase(
			Int32 month, Int32 day, Date date, Int32 expectedYear0, Int32[] expectedDayAdjustments)
		{
			Date[] expectedResults = expectedDayAdjustments
				.Select((adjustment, index) => new Date(expectedYear0 + index, month, day).AddDays(adjustment))
				.ToArray();
		...
		return
		[
			CreateTestCase(6, 15, new Date(1999, 1, 1), 2000, [0, 0, 2]),
			CreateTestCase(6, 15, new Date(2000, 1, 1), 2000, [0, 0, 2]),
			CreateTestCase(6, 15, new Date(2002, 6, 17), 2002, [2]),
			CreateTestCase(6, 15, new Date(2002, 6, 18), 0, []),
			CreateTestCase(6, 15, new Date(2003, 1, 1), 0, []),
			CreateTestCase(12, 30, new Date(2001, 1, 1), 2000, [2, 0, 0]),
			CreateTestCase(12, 30, new Date(2001, 12, 31), 2002, [0]),
		];
	}
